...
export function Sidebar({open,onClose,mode="demo"}:{open:boolean;onClose:()=>void;mode?:DataMode}) {
  ...
  return <><aside className={`sidebar ${open?"open":""}`}><div className="sidebar-brand"><Brand/><button onClick={onClose} aria-label="Close navigation">×</button></div><div className="workspace-switcher"><span>TS</span><div><b>TotalScope Intelligence</b><small>Restoration intelligence</small></div><em>⌄</em></div><nav><small>INTELLIGENCE</small>{main.map(([href,label,icon])=><Link key={href} href={href} onClick={onClose} className={path===href?"active":""}><i>{icon}</i>{label}</Link>)}<small>MANAGE</small><Link href="/admin/imports/q2-2026" onClick={onClose} className={path.startsWith("/admin/imports")?"active":""}><i>✓</i>Import validation</Link><Link href="/settings" onClick={onClose} className={path==="/settings"?"active":""}><i>⚙</i>Settings</Link><Link href="/client-dashboard" onClick={onClose} className={path==="/client-dashboard"?"active":""}><i>◎</i>Client View</Link></nav><div className="system-status"><div><span className="live-dot"/><b>{mode==="live"?"Authenticated archive":"Demo environment"}</b></div><small>{mode==="live"?"Staging · Stripe not connected":"Synthetic data · no live feeds"}</small></div><form action={signOut} className="user-card"><span>TS</span><div><b>Authorized user</b><small>{mode==="live"?"Authenticated staging session":"Explicit local demo"}</small></div><button type="submit" aria-label="Sign out" disabled={mode!=="live"}>Sign out</button></form></aside>{open&&<button className="sidebar-overlay" onClick={onClose} aria-label="Close navigation"/>}</>
}
