...
try{
  ...
  const locations=await browser.rpc("get_weather_internal_client_locations_v2");assert.ifError(locations.error);assert.ok(locations.data.length>0,"governed current-client coordinates must populate the internal Weather inventory");
  ...
  assert.equal(identities,863);assert.equal(contacts,692);assert.equal(people,671);assert.equal(aliases,804);assert.ok(ungeocoded>=0&&ungeocoded<863);
  ...
  console.log(JSON.stringify({passed:true,projectRef:PROJECT_REF,restrictedClients,restrictedBranches,statuses,identities,aliases,people,contacts,lineage,ungeocoded,currentWeatherLocations:locations.data.length,profileResolution:true,missingProfileSafe:true,importRunStatus:run.data.status,privacy:"aggregate_only"}));
}finally{
